...
<!-- Row container for entire Chat component -->
  <v-row class="fixedChat" v-click-outside="clickOutside">
      ...
      <v-expand-transition>
        ...
        <v-card v-if="chatShow" height="600" class="chatBox" >
            <v-slide-x-transition mode="out-in">
                <ChatConversation @back-to-rooms="backToRooms" :chatroom="selectedRoom" v-if="(window === 'chatConvo' && selectedRoom != null)  " />
    ...
    <v-col cols="12"  :class="$vuetify.breakpoint.mobile ? 'd-flex justify-end' : 'pt-0'" >
      <v-btn v-if="!$vuetify.breakpoint.mobile" class="pa-2" id="chatBtn" color="#131313"  block @click="chatShow = !chatShow" shaped>
      ...
      <v-btn v-else @click="chatShow = !chatShow" fab id="chatBtnFab">  <v-icon class=" msgIcon"> fas fa-comment-alt</v-icon> </v-btn>
    </v-col>
  </v-row>
</template>
